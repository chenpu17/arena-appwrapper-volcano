...
**Environment Variables (Volcano Job mode):**

| Variable | Value | Description |
|----------|-------|-------------|
| `MASTER_ADDR` | `{job}-{task}-0.{job}` | Master node DNS address |
| `MASTER_PORT` | `--master-port` value | Distributed communication port |
| `NNODES` | `--replicas` value | Number of nodes (Pods) |
